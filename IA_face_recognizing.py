...
#img = cv2.cvtColor(cv2.imread("lulus.jpg"), cv2.COLOR_BGR2RGB)

frame = cv2.imread("Reconhecimento_Facial/test2.jpeg")
...
while True:
    try:
        ...
        for i in range(len(detection)):
            cv2.rectangle(frame, (detection[i]["box"][0], detection[i]["box"][1]), 
                                (detection[i]["box"][0]+detection[i]["box"][2], detection[i]["box"][1]+detection[i]["box"][3]),
                                (0, 255, 0), 2) 

            for key in detection[i]["keypoints"]:
                cv2.circle(frame, detection[i]["keypoints"][key], 2, (0,255,0), 2)
                print()

        cv2.imshow("LULUS", frame)
        key = cv2.waitKey(1)
